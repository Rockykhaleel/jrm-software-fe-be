...
dotenv.config();
const Darul = require("../models/other_model");
const ObjectId = require("mongodb").ObjectId;
const CsvParser = require("json2csv").Parser;

module.exports = {
  addother: async (req, res) => {
    const { name, purpose, duration, outcome, budget, docomple, userId } =
      req.body;
  ...
  getReportData: async (req, res) => {
    try {
      ...
      userData.forEach((user) => {
        const { name, purpose, duration, outcome, budget, docomple } = user;
        users.push({
          name,
          purpose,
          duration,
  ...
  updateOrCreateother: async (req, res) => {

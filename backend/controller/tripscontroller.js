...
dotenv.config();
const Darul = require("../models/trips_model");
const ObjectId = require("mongodb").ObjectId;
const CsvParser = require("json2csv").Parser;

module.exports = {
  addtrips: async (req, res) => {
    const { places, date, nospasticipated, outcomes, expenses, userId } =
      req.body;
  ...
  getReportData: async (req, res) => {
    try {
      ...
      userData.forEach((user) => {
        const { places, date, nospasticipated, outcomes, expenses } = user;
        users.push({
          places,
          date,
          nospasticipated,
  ...
  updateOrCreatetrips: async (req, res) => {
